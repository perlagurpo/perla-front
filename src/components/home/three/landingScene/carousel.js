...
export default function Carrousel({ imagesSource=[] , yOffset=4 }) {
  ...
  return(
    <group position={[0,yOffset,0]}>
      ...
      <DisplayImagen position={[-4.5,0,4]} imgSource={imagesSource[0]} />
      <DisplayImagen position={[0,0,4]} imgSource={imagesSource[1]} />
      <DisplayImagen position={[4.5,0,4]} imgSource={imagesSource[2]} />
    </group>
  );
}

function DisplayImagen({ position, imgSource }) {
  const imgRef = useRef();
  const [hovered, setHovered] = useState(false);
  
  useFrame(
    (state, delta) => {
      imgRef.current.scale.y = damp(imgRef.current.scale.y, hovered ? 6 : 5, 8, delta);
      imgRef.current.scale.x = damp(imgRef.current.scale.x, hovered ? 4.5 : 3.75, 8, delta);
    }
  );
